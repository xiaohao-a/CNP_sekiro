...
function guid() {
        ...
        var client = new SekiroClient("wss://xxxxxxxx:5605/websocket?group=zhongshiyou&clientId=" + guid());



    client.registerAction("my_encrypt",function(request, resolve,reject ){
        // request.page为蜘蛛文件传递过来的页码请求参数
        userData.pageNo = request.page;
        let resultobj ={};
        // 调用原网页代码，生成随机的16位字符
        let randomStr = randomString(16);
        // 传输json字符串的关键字不要为data，否则只会传递data的数据
        // 调用AES对请求页面等信息加密，RSA对AES_key进行加密
        resultobj['page_data'] = Encrypt(JSON.stringify(userData),randomStr);
        resultobj['key_data'] = encrypt.encrypt(randomStr);
        let resultdata =  JSON.stringify(resultobj);
        // 返回加密后的结果
        resolve(resultdata);
    });
    client.registerAction("my_decrypt",function(request, resolve,reject ){
        let resultobj ={};
        // 调用解密-RSA
        var decrypt = new JSEncrypt();
        decrypt.setPrivateKey(privateKey);
        var aesKey = decrypt.decrypt(request['encrypted']);
        // 调用解密-AES
        var dataStr = Decrypt(request['request_data'],aesKey).replace(/\r\n/g,"").replace(/\\/g,"");
        resultobj['aesKey'] = aesKey;
        resultobj['dataStr'] = dataStr;
        let resultdata =  JSON.stringify(resultobj);
        // 返回结果
        resolve(resultdata);
    });
